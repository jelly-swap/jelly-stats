...
import { aggregateVolumeDates, safeAccess, getDayOnly, selectorStyles } from '../../utils';
...
export default () => {
  ...
  useEffect(() => {
    if (ethVolume.length > 0) {
      setDatesForChart(aggregateVolumeDates(ethVolume, safeAccess(prices, ['USDT'])));
    }
  }, [ethVolume, prices]);
  ...
  const tooltips = {
    ...
    callbacks: {
      label: (tooltipItem, data) => {
        ...
        return `Volume: ${calcTokenAmount(network, amountInUsd)} ${network} (${amountInUsd} $)`;
      },
      title: tooltipItem => {
